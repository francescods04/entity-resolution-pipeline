...
This document details the scientific and engineering improvements that make this entity resolution pipeline research-grade.
...
### Challenge
Standard blocking fails when companies have significantly different legal names (e.g., *Deliveroo* vs. *Roofoods Ltd*). This is the primary cause of low recall in corporate matching.
...
1.  **Generative AI Enrichment (Perplexity API)**:
    -   We queried LLMs with browsing capabilities to identify "Parent Company of [Brand Name]" and "Legal Entity Name for [Brand Name]".
    -   *Sheet*: `Matching ai`
    -   *Contribution*: Solved complex rebranding cases (e.g., *Taxify* → *Bolt Technology OÜ*).
...
### Challenge
Previously, disambiguating between multiple Orbis candidates (e.g., "McDonald's Ltd - UK" vs "McDonald's Ltd - IRE") relied on hardcoded heuristic scores. This is scientifically weak.
...
3.  **Confidence Intervals**: All metrics (Precision/Recall) are reported with **Wilson Score Intervals** (e.g., 95.2% ±1.3%), providing statistical significance.
...
### Contribution
To justify the complexity of the pipeline, one must prove each component contributes value.
- **Framework**: `src/research_analytics.py` includes an `AblationStudy` class.
- **Output**: Generates a standard table showing performance drops when features are removed (e.g., "Removing `AliasBlocking` causes -15% Recall").
